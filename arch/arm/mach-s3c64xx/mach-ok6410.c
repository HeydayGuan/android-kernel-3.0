...
static struct s3c_fb_pd_win ok6410_fb_win[] = {
	{
	...
	while (*fp) {
		...
		case '0'...'9':	/* tft screen */
			if (features->done & FEATURE_SCREEN) {
				...
				int li = f - '0';
				if (li >= ARRAY_SIZE(ok6410_fb_win))
...
static void __init ok6410_machine_init(void)
{
	...
	ok6410_lcd_pdata.win[0] = &ok6410_fb_win[features.lcd_index];

	printk(KERN_INFO "OK6410: selected LCD display is %dx%d\n",
		ok6410_lcd_pdata.win[0]->win_mode.xres,
		ok6410_lcd_pdata.win[0]->win_mode.yres);
